...
public class BaseRotator : DragableObject
{
    ...
    {
        transform.rotation = Quaternion.Euler(0, GetCurrentMouseOffset().x - GetInitialMouseOffset().x + initialRotation, 0);
    }
    ...
    {
        currentJoystickValue = Vector3.SmoothDamp(currentJoystickValue, new Vector3(Input.GetAxis("HatHorizontal"), 0, 0), ref currentVelocity, smoothTime);
        transform.rotation = Quaternion.Euler(0, transform.rotation.eulerAngles.y + (-currentJoystickValue.x * speedFactor), 0);
    }
