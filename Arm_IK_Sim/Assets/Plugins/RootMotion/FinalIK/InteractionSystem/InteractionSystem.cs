...
namespace RootMotion.FinalIK {
	...
	public class InteractionSystem : MonoBehaviour {
		...
		[SerializeField] private Vector3 _interactionPointOffset = Vector3.zero;
		private Vector3 interactionPoint => transform.TransformPoint(_interactionPointOffset);
		...
		/// </summary>
		public List<InteractionObject> GetSortedInteractionObjectsInRange()
		{
			List<InteractionObject> interactionObjects = new List<InteractionObject>();
			if (!IsValid(true) || triggersInRange.Count == 0)
			{
				return interactionObjects;
			}
			
			// Sort all valid trigger "ranges" by distance.
			Dictionary<InteractionTrigger.Range, float> distancesDict = new Dictionary<InteractionTrigger.Range, float>();
			for (int i = 0; i < triggersInRange.Count; i++)
			{
				if (triggersInRange[i] != null && triggersInRange[i].ranges[bestRangeIndexes[i]] != null)
				{
					float sqrMag = Vector3.SqrMagnitude(triggersInRange[i].transform.position - interactionPoint);
					distancesDict.Add(triggersInRange[i].ranges[bestRangeIndexes[i]], sqrMag);
				}
			}
			List<KeyValuePair<InteractionTrigger.Range, float>> sortedDictList = distancesDict.ToList();
			sortedDictList.Sort((pair1, pair2) => pair1.Value.CompareTo(pair2.Value));
			
			// Extract the InteractionObjects from the sorted trigger ranges
			foreach (KeyValuePair<InteractionTrigger.Range,float> pair in sortedDictList)
			{
				HashSet<InteractionObject> objs = new HashSet<InteractionObject>();
				for (int i = 0; i < pair.Key.interactions.Length; i++) 
				{
					if (pair.Key.interactions[i].interactionObject != null)
					{
						objs.Add(pair.Key.interactions[i].interactionObject);
					}
				}
				interactionObjects.AddRange(objs);
			}
